...
sub cache_init {
    ...
    # Change $servers to an arrayref if its not already.
    $servers = [ $servers ] if (ref($servers) ne 'ARRAY');
    ...
    $gobj->cache_servers(join(',', @$servers));
...
sub cache_handle {
    ...
    if ($p) {
        $cache_handles->{$gobj->cache_servers()} = $p;
    }

    return($cache_handles->{$gobj->cache_servers()});
...
sub cache_servers {
    ...
    $$gobj{__CACHE_SERVERS__} = $servers if defined $servers;
    return($$gobj{__CACHE_SERVERS__});
...
This method sets or returns a comma separated list of cache servers. It is used
by cache_handle as key into the cache_handles hash - this is so every set of
cache servers will have its own cache handle that will remain persistent.
