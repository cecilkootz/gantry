package Gantry::Utils::DBConnHelper::MP20;
...
sub persistent_dbh {
    my $r           = 'Apache2::RequestRec';
    my $gantry_conf = _get_gantry_conf( $r );
    my $persistent  = $gantry_conf->{ 'persistent_dbh' };
    
    unless ( defined $persistent ) {
        $persistent = $r->dir_config( 'persistent_dbh' );
    }
...
This module is designed to work with mod_perl 2.0.  There is another
module for mod_perl 1.3.
...
methods here.  All values are taken from PerlSetVars.
...
=item persistent_dbh
  ...
  Return 0 or 1 to indicate if a persistent database connection should be
  maintained. This option can be set in the gantry conf
  for your application. Set persistent_dbh to 0 to disable persistence.
  The default is 1.
