...
sub handler : method {
    ...
    # Call cleanup. Log any errors.
    eval {
        $self->cleanup();
    };
    
    if ( $@ ) {
        $self->log_error( "Error calling cleanup: $@" );
    }
...
sub cleanup {
    ...
    my $helper;
    my $auth_dbh;
    my $persistent_dbh = $self->can('persistent_dbh')
        ? $self->persistent_dbh()
        : 0;

    foreach my $sh ( keys %{ $self->{__SCHEMAS__} } ) {
        unless ( $self->{__SCHEMAS__}->{$sh}->storage->dbh->{AutoCommit} ) {
            $self->{__SCHEMAS__}->{$sh}->txn_rollback();
        }

        unless ( $persistent_dbh ) {
            $self->{__SCHEMAS__}->{$sh}->storage->disconnect();
            delete $self->{__SCHEMAS__}->{$sh};
        }
    }

    # Get auth db handle
    $helper   = Gantry::Utils::DBConnHelper->get_subclass();
    $auth_dbh    = $helper->get_auth_dbh();

    if ( $auth_dbh ) {
        unless ( $auth_dbh->{AutoCommit} ) {
            $auth_dbh->rollback();
        }
